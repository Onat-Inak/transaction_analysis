...
class GRU(nn.Module):
    def __init__(self, transaction_size, embedding_dim, hidden_dim, num_layers, device, num_classes = 1, batch_first = True, fc_hidden_dim = 128):
        ...
        # initialize embedding, gru and fully connected layers
    ...
    def forward(self, x):
        ...
        h_t, _ = self.gru(x_embed.to(self.device), h0) # (batch_size, sequence_length/ number of total hidden states, hidden_dim)
        # h_t = self.ln2(h_t)
